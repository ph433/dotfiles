...
typedef struct {
	...
	#endif // CENTER_PATCH
	int isfloating;
	int isfullscreen; /* <--- BẠN THÊM VÀO ĐÂY */
...
#if BAR_ALPHA_PATCH
        tagscheme[i] = drw_scm_create(drw, tagsel[i], alphas[SchemeTagsNorm], 2);
#else
        tagscheme[i] = drw_scm_create(drw, tagsel[i], 2);
#endif // BAR_ALPHA_PATCH

    /* --- BẠN BẮT ĐẦU PASTE THÊM TỪ ĐOẠN NÀY --- */
    tagnormscheme = ecalloc(LENGTH(tagnorm), sizeof(Clr *));
    for (i = 0; i < LENGTH(tagnorm); i++)
#if BAR_ALPHA_PATCH
        tagnormscheme[i] = drw_scm_create(drw, tagnorm[i], alphas[SchemeTagsNorm], 2);
#else
        tagnormscheme[i] = drw_scm_create(drw, tagnorm[i], 2);
#endif // BAR_ALPHA_PATCH
