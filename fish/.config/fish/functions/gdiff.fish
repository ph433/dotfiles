function gdiff --description "FZF Git Diff All-in-One Dashboard (Tích hợp gọi glog)"
    ...
    # 3. Lệnh Dò Commit bằng cách gọi hàm glog (Ctrl-Shift-Left)
    set -l history_cmd "fish -c '
        # [THỦ THUẬT] Tạo hàm commandline giả để bắt output từ glog
        ...
        # Gọi hàm glog của bạn (đòi hỏi cấp lại luồng terminal để vẽ fzf)
        glog </dev/tty >/dev/tty

        # Xử lý kết quả trả về
        if test -f /tmp/glog_hash
            set hash (cat /tmp/glog_hash)
            rm /tmp/glog_hash
            
            # Nếu glog trả về nhiều hash, chỉ lấy cái đầu tiên để restore
