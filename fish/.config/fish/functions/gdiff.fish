function gdiff --description "FZF Git Diff All-in-One Dashboard"
    ...
    # ---------------------------------------------------------
    # Xử lý chuỗi tên file từ git status (hỗ trợ cả trường hợp đổi tên R old -> new)
    ...
    # ---------------------------------------------------------
    # KHỞI CHẠY FZF (CHỈ CÓ 1 TẦNG DUY NHẤT)
    # ---------------------------------------------------------
    set -l fzf_output (git -c color.status=always status -s | fzf \
        ...
        --tiebreak=index \
        --multi \
        --header="[Git Diff]  Right: Add |  Left: Unstage |  Ctrl-Right: Commit |  Ctrl-F: Full Màn" \
        ...
        --bind='ctrl-f:execute(less -R < /dev/tty > /dev/tty {-1})' \
        --expect=ctrl-right)

    # Thoát an toàn nếu bấm Esc (không có output trả về)
    if test (count $fzf_output) -eq 0
        commandline --function repaint 2>/dev/null
        return 0
    end

    # Lấy thông tin phím đã bấm (để thoát ra) và các file được đánh dấu
    ...
    if test "$key_pressed" = "ctrl-right"
        # Nếu đang bôi đỏ/xanh vài file bằng Tab mà ấn Ctrl-Right, tiện tay Add luôn
        ...
        # In sẵn lệnh commit ra Terminal, nhét con trỏ vào giữa 2 dấu ngoặc kép
