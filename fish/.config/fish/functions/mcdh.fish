...
function mcdh --description "Menu based cd command"
    ...
    echo ""                          # Tạo một dòng trống để cách ly với prompt cũ
    commandline -f clear-screen       # Ép Terminal xóa sạch các ký tự rác đang hiển thị lỗi
    ...
    read -n 1 -l -p "echo '$msg'" choice
    echo "" # Thêm dòng này để tạo khoảng xuống dòng sạch sẽ sau khi bấm nút
    if test -z "$choice"
        return 0
