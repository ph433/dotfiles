function t
    ...
    # 2. Tạo script Preview (Hiển thị ngay lập tức, dùng scaler chuẩn)
...
# Giữ khung trống cho FZF không bị sập preview
...
if [[ -n "$vid_id" && -p "$FIFO_UEBERZUG" ]]; then
    ...
    draw_image() {
        if [[ "$(cat "$current_vid_file" 2>/dev/null)" == "$vid_id" ]]; then
            # fit_contain giúp ảnh to hết cỡ giữ nguyên tỉ lệ 16:9, cực nét
    ...
    # Nếu ảnh đã được tải ngầm xong -> Vẽ luôn không delay
    if [[ -f "$img_path" ]]; then
        draw_image
    else
        # Fallback: Đề phòng mạng chậm, ảnh chưa tải kịp khi người dùng cuộn tới
        (
...
' > "$preview_script"
    ...
    # 3. Lấy data và nạp vào FZF (CÓ PRE-FETCH)
    set -l selected (yt-dlp "ytsearch20:$argv" \
        ...
        | awk -F '\t' -v OFS='\t' -v tmp="$tmp_dir" '
            ...
            {
                # KÍCH HOẠT PRE-FETCH CỰC MẠNH: Tải ngầm ảnh MaxRes, hỏng thì lấy HQ
                id = $6
                system("(curl -s -f \"https://img.youtube.com/vi/" id "/maxresdefault.jpg\" -o \"" tmp "/" id ".jpg\" || curl -s \"https://img.youtube.com/vi/" id "/hqdefault.jpg\" -o \"" tmp "/" id ".jpg\") >/dev/null 2>&1 &")
